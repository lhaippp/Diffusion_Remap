...
class TanoSave():
    def __init__(self, exp_id):
        super().__init__()
        time_str = time.strftime("%Y-%m-%d_/id-H%H", time.localtime()).split('_')  #%M%S
        date_dir = 'Outputs/log/' + time_str[0]
        if not os.path.exists(date_dir):
            os.makedirs(date_dir)
        log_path = date_dir + time_str[1].replace('id', str(exp_id)) + '.txt'
        with open(log_path, 'w') as log:
            log.write('Training records: \n')
        self.log_path = log_path

    def print(self, log_str):
        print(log_str)
...
if __name__ == '__main__':
    h, w = ts_img1.shape[-2:]
    tenMetric = torch.ones(1, 1, h, w).cuda()
    img1 = ts_img1.unsqueeze(0).cuda()
    flow = ts_flow.permute(2, 0, 1).contiguous().unsqueeze(0).cuda()  # from (h, w, 2) to (1, 2, h, w)
    img1_wp = softsplat.FunctionSoftsplat(tenInput=img1, tenFlow=flow, tenMetric=tenMetric, strType='softmax')
    torchvision.utils.save_image(img1_wp/img1_wp.max(), dir_save+'img1to2_warp.png')
